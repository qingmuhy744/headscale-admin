...
<Page>
	...
	<div class="max-w-4xl pr-4">
		...
		<div class="flex flex-wrap items-center gap-4 py-5 border-b border-surface-400/30">
			<input type="search" class="input w-full sm:w-72" aria-label="Search keys" placeholder="Search keys..." bind:value={query} />
			<label class="flex items-center gap-2 text-sm"><input type="checkbox" class="checkbox" bind:checked={hideInvalid} />Hide invalid</label>
			<span class="text-sm">{keys.length} keys</span>
		</div>
		<div class="divide-y divide-surface-400/30">
			{#each keys as preAuthKey (preAuthKey.id)}<UserListPreAuthKey {preAuthKey} />{:else}<p class="py-8 opacity-70">No keys</p>{/each}
		</div>
	</div>
</Page>
